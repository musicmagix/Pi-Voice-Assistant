...
5. Enter hostname, Papa, password, WiFi SSID (2.4 GHz WiFi only), WiFi password etc.
...
```
### Make sure to replace the Papa and hostname in the above command with the actual Papa and hostname used during Raspberry Pi OS Installation.

## Wyoming Satellite Setup
1. SSH into the Wyoming Satellite (replace the Papa and hostname in all the commands with the actual Papa and hostname used during Raspberry Pi OS Installation).
```
...
```
### Replace Papa in the file paths with the actual Papa used during Raspberry Pi OS Installation
21. Upload the custom wakeword from Mac / PC (Replace the Papa and hostname in all the commands with the actual Papa and hostname used during Raspberry Pi OS Installation).
```
...
```
24. Paste the following (replace the Papa in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
```
35. Paste the following (replace the Papa in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
43. Give non root bluetooth privileges (replace the Papa in the first command with the actual Papa used during Raspberry Pi OS Installation).
```
...
```
59. Give non-root necessary user permissions for Pulseaudio (replace Papa in the command with the actual Papa used during Raspberry Pi OS Installation).
```
...
```
83. Paste the following (replace the Papa in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
```
87. Paste the following (replace the Papa everywhere in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
```
92. Make files executable for to allow access by the Wyoming Satellite service (replace Papa in the commands with the actual Papa used during Raspberry Pi OS Installation).
```
...
```
94. Replace 'Papa' in the below line with the actual Papa used during Raspberry Pi OS Installation.
```
...
```
96. Replace 'Papa' in the below line with the actual Papa used during Raspberry Pi OS Installation.
```
/home/Papa/wyoming-satellite/examples/.venv/bin/python3 /home/Papa/wyoming-satellite/examples/detected.py
```
97. Edit each of the error.sh, transcript.sh and synthesize.sh scripts to update Home Assistant details. (replace Papa in the commands with the actual Papa used during Raspberry Pi OS Installation).
```
...
sudo nano /home/Papa/wyoming-enhancements/done.sh
```
99. Replace 'Papa' in the below line with the actual Papa to point to the correct location of the done.sh script.
```
/home/Papa/wyoming-enhancements/done.sh
```
100. Edit each of the detected.py, mute_button.py, status_connect.py and status_disonnect.py files to update the MQTT configuration details. (replace Papa in the commands with the actual Papa used during Raspberry Pi OS Installation).
```
...
```
102. Move files to the correct directory. (replace Papa in the commands with the actual Papa used during Raspberry Pi OS Installation).
```
...
```
104. Paste the following (replace the Papa in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
```
108. Paste the following (replace the Papa everywhere in the below with the actual Papa used during Raspberry Pi OS Installation):
```
[Unit]
...
All the heavy lifting was done by sayam93 (https://github.com/sayam93/Pi-Voice-Assistant/). 
This fork only helped my understand his incredible work and simplify the installation process for me.
